...
(function(){
'use strict';
const VERSION='6.0.0-store-browser.26';
const SEARCH={
 ...
 setTimeout(function(){
   try{
     ...
     if(!usable.length){
       ...
       processing.innerHTML='<div class="v6-auto-processing-title">هوش‌یار در حال پردازش درخواست توست...</div><button type="button" class="v6-auto-processing-toggle" aria-expanded="false"><span class="v6-auto-processing-chevron" aria-hidden="true">›</span><span class="v6-auto-processing-done">هوش یار به مدت ۷ ثانیه پردازش کرد</span></button><div class="v6-auto-processing-details"><ul></ul></div>';
       const finalDetails=processing.querySelector('.v6-auto-processing-details');
       const finalList=finalDetails.querySelector('ul');
       const toggle=processing.querySelector('.v6-auto-processing-toggle');
       toggle.addEventListener('click',function(){const open=this.getAttribute('aria-expanded')==='true';this.setAttribute('aria-expanded',String(!open));finalDetails.classList.toggle('is-open',!open);});
       if(processingList)Array.from(processingList.children).forEach(function(item){finalList.appendChild(item);});
     ...
     const finalDetails=processing.querySelector('.v6-auto-processing-details');
     const finalList=finalDetails.querySelector('ul');
     processing.querySelector('.v6-auto-processing-toggle').addEventListener('click',function(){
       const open=this.getAttribute('aria-expanded')==='true';
       this.setAttribute('aria-expanded',String(!open));
       finalDetails.classList.toggle('is-open',!open);
     });
     const typedItems=processingList?Array.from(processingList.children):[];
     typedItems.forEach(function(item){finalList.appendChild(item);});
     box.style.display='block';
