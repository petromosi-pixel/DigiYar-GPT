...
.v5-smart-search-field #v5SmartSearchHint.v7-hooshyar-hint{display:block!important;right:48px!important;left:48px!important;top:50%!important;transform:translateY(-50%)!important;width:auto!important;height:34px!important;margin:0!important;padding:0!important;text-align:right!important;font-size:12px!important;font-weight:600!important;white-space:nowrap!important;overflow:hidden!important;z-index:1!important;pointer-events:none!important;color:#7f8da3!important}
.v7-hooshyar-hint-inner{position:relative!important;display:block!important;width:100%!important;height:34px!important;direction:rtl!important;padding:0!important;box-sizing:border-box!important}
.v7-hooshyar-hint-text{position:absolute!important;right:8px!important;left:8px!important;top:50%!important;display:block!important;width:auto!important;max-width:none!important;transform:translate(8px,-50%)!important;overflow:hidden!important;text-overflow:clip!important;white-space:nowrap!important;visibility:visible!important;opacity:0!important;color:inherit!important;animation:v7HooshyarSpeak .65s cubic-bezier(.22,.61,.36,1) forwards!important}
.v7-hooshyar-robot-zone{position:absolute!important;right:0!important;left:auto!important;top:0!important;width:40px!important;height:100%!important;display:flex!important;align-items:center!important;justify-content:center!important;box-sizing:border-box!important;z-index:4!important;pointer-events:none!important;animation:v7BotFloat 3.8s ease-in-out infinite!important}
.v7-hooshyar-robot-zone img{display:block!important;width:30px!important;height:30px!important;object-fit:contain!important;overflow:visible!important}
.v7-hooshyar-action-zone{position:absolute!important;left:0!important;right:auto!important;top:0!important;width:40px!important;height:100%!important;display:flex!important;align-items:center!important;justify-content:center!important;box-sizing:border-box!important;z-index:4!important}
.v7-hooshyar-action-zone .v5-smart-search-submit,.v7-hooshyar-action-zone .v5-smart-search-clear{position:static!important;left:auto!important;right:auto!important;top:auto!important;transform:none!important;width:34px!important;height:34px!important;margin:0!important;border-radius:10px!important;flex:none!important}
...
function init(){const form=document.getElementById('v5SmartSearchForm'),input=document.getElementById('v5SmartSearchInput'),hint=document.getElementById('v5SmartSearchHint'),clear=document.getElementById('v5SmartSearchClear'),submit=form.querySelector('.v5-smart-search-submit');if(!form||!input||!hint)return;installCardStyle();ensureHooshyarZones();const stale=document.getElementById('v5SmartSearchResults');if(stale)stale.remove();function ensureHooshyarZones(){
  const field=form.querySelector('.v5-smart-search-field');
  if(!field)return;
  let robotZone=field.querySelector('.v7-hooshyar-robot-zone');
  if(!robotZone){
    robotZone=document.createElement('div');
    robotZone.className='v7-hooshyar-robot-zone';
    robotZone.setAttribute('aria-hidden','true');
    robotZone.innerHTML='<img src="assets/hooshyar/friendly-robot.svg" alt="" draggable="false">';
    field.appendChild(robotZone);
  }
  let actionZone=field.querySelector('.v7-hooshyar-action-zone');
  if(!actionZone){
    actionZone=document.createElement('div');
    actionZone.className='v7-hooshyar-action-zone';
    actionZone.setAttribute('aria-hidden','true');
    field.appendChild(actionZone);
  }
  actionZone.appendChild(submit);
  actionZone.appendChild(clear);
}
function renderHint(){const h=hints[i];hint.classList.add('v7-hooshyar-hint');hint.classList.remove('v6-hooshyar-brand');hint.innerHTML='<div class="v7-hooshyar-hint-inner"><div class="v7-hooshyar-hint-text">'+h.text+'</div></div>';}let searched=false;function syncHint(){const hasText=!!input.value.trim();const textEl=hint.querySelector('.v7-hooshyar-hint-text');hint.style.opacity='1';hint.style.visibility='visible';hint.style.display='block';if(textEl){textEl.style.display=hasText?'none':'block';textEl.style.visibility=hasText?'hidden':'visible';textEl.style.opacity=hasText?'0':'1';}}function syncActions(){const hasText=!!input.value.trim();if(submit)submit.hidden=!hasText||searched;if(clear)clear.hidden=!searched;}input.addEventListener('input',function(){searched=false;syncHint();syncActions();});input.addEventListener('focus',syncHint);input.addEventListener('blur',syncHint);if(clear)clear.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();input.value='';input.disabled=false;input.placeholder='';searched=false;i=0;renderHint();syncHint();syncActions();hint.classList.remove('v5-hint-show');void hint.offsetWidth;hint.classList.add('v5-hint-show');var r=document.getElementById('v5SmartSearchResults');if(r)r.remove();var s=document.getElementById('v6StoreSimulatorResults');if(s)s.remove();var sim=document.querySelector('[data-v6-store-simulator]');if(sim)sim.remove();input.focus();});renderHint();syncHint();syncActions();timer=setInterval(()=>{if(!input.value.trim()){i=(i+1)%hints.length;hint.classList.remove('v5-hint-show');void hint.offsetWidth;renderHint();syncHint();hint.classList.add('v5-hint-show');}},2600);form.addEventListener('submit',async e=>{e.preventDefault();const q=input.value.trim();if(!q)return;const textEl=hint.querySelector('.v7-hooshyar-hint-text');if(textEl){textEl.style.display='none';textEl.style.visibility='hidden';textEl.style.opacity='0';}const old=input.placeholder;input.placeholder='دارم برات جستجو می‌کنم...';input.disabled=true;try{const browser=await loadStoreBrowser();browser.open(q);}catch(err){console.error('DigiYar Hooshyar Store Browser:',err);}finally{input.disabled=false;input.placeholder=old;searched=true;syncHint();syncActions();}});}
