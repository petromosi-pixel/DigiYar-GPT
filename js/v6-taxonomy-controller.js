...
(function(){
  'use strict';
  var STORES=['digikala','snappshop','torob','basalam','banimode','modiseh','technolife','meghdadit','janebi','digiland'];
  ...
  function bind(){var store=$('storeSelect'),cat=$('v5Category');if(!store||!cat)return false;ensureMeghdadStore();ensureDigilandStore();if(!store.dataset.v6UnifiedBound){store.dataset.v6UnifiedBound='1';store.addEventListener('change',function(){setTimeout(normalizeCategory,0);if(store.value==='digiland')setTimeout(mountDigiland,80)})}if(!cat.dataset.v6UnifiedBound){cat.dataset.v6UnifiedBound='1';cat.addEventListener('change',function(){setTimeout(normalizeCategory,0);if(store.value==='digiland')setTimeout(mountDigiland,40)})}cleanSelect(cat);normalizeCategory();loadProductOptions();if(window.DigiYarV6ProductOptionsReady){loadTaxonomies();setTimeout(mountDigiland,80);loadScript('js/v6-category-budget-brand-fix.js')}return true}
