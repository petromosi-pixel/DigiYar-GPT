...
(function () {
  ...
  function fillForm(profile){if(!profile)return;const d=profile.declared||{};if($("category"))$("category").value=d.category||"general";if($("budgetMax"))$("budgetMax").value=d.budget&&d.budget.max?d.budget.max:"";if($("priorities"))$("priorities").value=(d.priorities||[]).join("، ");if($("usage"))$("usage").value=d.usage||"";if($("requirements"))$("requirements").value=(d.requirements||[]).join("، ");if($("constraints"))$("constraints").value=(d.constraints||[]).join("، ");}
  async function renderProfile(profile){const summary=$("needSummary"),box=$("recommendations"),hint=$("resultHint");if(!summary||!box||!hint)return;summary.className="need-summary empty";summary.textContent="";if(!profile){box.innerHTML="";hint.textContent="برای شروع اطلاعات خریدت رو وارد کن.";return;}if(!window.DigiYarNeedEngine){hint.textContent="موتور تحلیل نیاز در دسترس نیست.";return;}let need;try{need=window.DigiYarNeedEngine.buildNeedFromProfile(profile);}catch(error){console.error("DigiYar Need Engine:",error);hint.textContent="خطا در ساخت نیاز خرید.";return;}if(!need)return;hint.textContent="در حال پیدا کردن کالاهای مناسب...";box.innerHTML='<div class="recommendation-loading">در حال جستجوی کالاهای واقعی...</div>';const recommendations=await getRecommendations(need);if(!recommendations.length){box.innerHTML='<div class="need-summary">برای این نیاز هنوز پیشنهاد مناسبی پیدا نشد.</div>';hint.textContent="اطلاعات بیشتری وارد کن تا پیشنهادهای دقیق‌تری ساخته شود.";return;}box.innerHTML=recommendations.map(renderRecommendationCard).join("");hint.textContent="بر اساس اولویت‌های انتخابی تو به ترتیب زیر پیشنهاد میشن";}
  const profileForm=$("profileForm");if(profileForm)profileForm.addEventListener("submit",async function(event){event.preventDefault();if(!window.DigiYarUserProfile)return;try{const profile=window.DigiYarUserProfile.normalize({category:$("category")?$("category").value:"",budgetMax:$("budgetMax")?$("budgetMax").value:"",priorities:$("priorities")?$("priorities").value:"",usage:$("usage")?$("usage").value:"",requirements:$("requirements")?$("requirements").value:"",constraints:$("constraints")?$("constraints").value:""});window.DigiYarUserProfile.save(profile);await renderProfile(profile);const resultSection=$("resultSection");if(resultSection)resultSection.scrollIntoView({behavior:"smooth",block:"start"});}catch(error){console.error("DigiYar Profile:",error);}});
  ...
  if(installDismiss)installDismiss.addEventListener("click",function(){installPromptDismissed=true;window.clearTimeout(installPromptTimer);window.clearTimeout(installPromptDelayTimer);hideInstallPrompt();});
  renderPlatforms();let savedProfile=null;if(window.DigiYarUserProfile){try{savedProfile=window.DigiYarUserProfile.getProfile();}catch(error){savedProfile=null;}}fillForm(savedProfile);renderProfile(savedProfile);
})();
